...
void execute_program(int second,char *arg){	// In here i declared a function called execute_program which gets parameters time and program name.
	int child_status;			// In here i declared an integer variable which knows child processes status.
	if(fork() == 0){			// In here i called fork in an if condition and if process is child process :
...
int main(int argc, char **argv){		// In here i declared main function which contains main program codes.
	...
	while(argv[1][i]){			// In here i called while loop for counting the length of 2. command line parameter which contains program name.
		i++;
		boyut = i;
	}
	...
	for(int i=0;i<=boyut+2;i++){		// In here i started a for loop for getting program name with "./" string.
		if(i == 0) argvs[i] = '.';	// In here i assigned the '.' character to char array.
		else if(i == 1) argvs[i] = '/';	// In here i assigned tge '/' character to char array.
		else argvs[i] = argv[1][i-2];	// In here i assigned the program name after assigning "./" string.
	}
